...
function parseCard($, el) {
  ...
  const bedsText = $card.text().match(/(\d+)\s*bed/i)?.[1];
  const bathsText = $card.text().match(/(\d+)\s*bath/i)?.[1];
...
export async function scrapeBuyRentKenya({ pages = 1 } = {}) {
  const results = [];
  for (let p = 1; p <= pages; p++) {
    // Try different URL patterns
    const urls = [
      `${BASE}/property-for-sale?page=${p}`,
      `${BASE}/properties-for-sale?page=${p}`,
      `${BASE}/listings?page=${p}`,
    ...
    for (const url of urls) {
      try {
        ...
        cards.each((_, el) => {
          ...
          console.log('Parsed card:', { url: card.url, title: card.title, price: card.price });
          if (card.url && card.title) {
            results.push(card);
          } else {
            console.log('Card rejected - missing URL or title');
          }
        });
