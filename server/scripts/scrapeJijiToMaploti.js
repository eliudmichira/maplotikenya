/**
 * Scrape Jiji.co.ke and push listings to the MAPLOTIKENYA Firestore project.
 * Uses firebase-admin (service account) — bypasses security rules.
 *
 * Usage:
 *   node scripts/scrapeJijiToMAPLOTIKENYA.js                  # default: 1 page rent, 0 sale, dry-run=false
 *   DRY_RUN=1 node scripts/scrapeJijiToMAPLOTIKENYA.js        # scrape + map but DO NOT write
 *   PAGES_RENT=10 PAGES_SALE=5 node scripts/scrapeJijiToMAPLOTIKENYA.js
 */
...
// â”€â”€â”€ Init Firebase Admin â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
console.log(`ðŸ”‘ Firestore project: ${serviceAccount.project_id}`);

// â”€â”€â”€ Coord lookup (reused from existing scrapeToFirestore.js) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const AREA_COORDS = {
...
// â”€â”€â”€ Mapping: Jiji â†’ MAPLOTIKENYA schema â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function toMAPLOTIKENYAProperty(listing, coords) {
...
// â”€â”€â”€ Dedupe â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function loadExistingSourceUrls() {
  console.log('ðŸ”Ž Loading existing jiji sourceUrls for dedupe...');
  ...
  console.log(`   ${set.size} existing jiji listings on MAPLOTIKENYA`);
  return set;
}
...
async function main() {
  ...
  console.log(`\nðŸ•·ï¸  Scraping Jiji  |  rent=${pagesRent} sale=${pagesSale} land=${pagesLand} commercial=${pagesCommercial}  |  dryRun=${dryRun}`);
  ...
  console.log(`\nðŸ“¦ ${listings.length} unique listings scraped`);
  ...
  for (const listing of listings) {
    ...
    try {
      ...
      const doc = toMAPLOTIKENYAProperty(listing, coords);
      if (dryRun) {
        added++;
      } else {
      ...
      process.stdout.write(`\r  âœ… ${added} added | â­ï¸  ${skipped} skipped | âŒ ${failed} failed`);
    } catch (e) {
      failed++;
      console.error(`\n   âŒ ${listing.url}: ${e.message}`);
    }
  }

  console.log(`\n\nðŸŽ‰ Done!`);
  console.log(`   âœ… Added:   ${added}${dryRun ? ' (DRY RUN — not written)' : ''}`);
  console.log(`   â­ï¸  Skipped: ${skipped} (already in Firestore)`);
  console.log(`   âŒ Failed:  ${failed}`);
  const gs = geocodeStats();
  console.log(`   ðŸ“ Geocode: ${gs.hits} resolved, ${gs.misses} unresolved (${gs.cached} cached for next run)`);
  process.exit(0);
}

main().catch(e => {
  console.error('âŒ Fatal:', e);
  process.exit(1);
});
