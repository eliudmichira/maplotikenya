...
const RTDB_BASE = 'https://maploti-default-rtdb.firebaseio.com/listings.json';
...
async function main() {
  ...
  try {
    ...
    console.error('  ❌ Auth error:', authErr.response?.data || authErr.message);
    return;
  }
  ...
  let inserted = 0;
  const targetUrl = `${RTDB_BASE}?auth=${idToken}`;
  ...
  for (const item of rawListings) {
    try {
      ...
      const res = await axios.post(targetUrl, listingData);
      if (res.status === 200) {
        inserted++;
  ...
  console.log(`\n🎉 SUCCESS! Inserted ${inserted} Mbanyu property listings into Maploti Realtime Database!`);
}
