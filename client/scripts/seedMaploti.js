// Seed script for MaplotiKenya Firebase project
...
// We call setupAuth in seedMaplotiKenya before anything else.
...
const sampleAgents = [
    {
        ...
        phone: "+254 700 123 456",
        image: "https://images.unsplash.com/photo-1494790108755-2616b612b786?w=150&h=150&fit=crop",
        bio: "Official MaplotiKenya Support Agent.",
...
async function seedProperties(adminUserId) {
    ...
    for (const property of sampleProperties) {
        try {
            ...
            console.log(`âœ… Property added with ID: ${docRef.id}`);
        } catch (error) {
...
async function seedAgents(uid) {
    ...
    if (sampleAgents.length > 0 && uid) {
        ...
        try {
            ...
            console.log(`âœ… Agent added with ID: ${uid}`);
        } catch (error) {
            console.error('âŒ Error adding agent:', error);
        }
    }
}
...
async function createTestUsers() {
    ...
    const testUsers = [
        { email: process.env.SEED_USER_EMAIL || 'user@maplotikenya.com', password: process.env.SEED_USER_PASSWORD || 'YOUR_USER_PASSWORD', role: 'user' },
        { email: process.env.SEED_ADMIN_EMAIL || 'support@maplotikenya.com', password: process.env.SEED_ADMIN_PASSWORD || 'YOUR_ADMIN_PASSWORD', role: 'admin' } // Create admin LAST to stay signed in
    ];
        ...
        try {
            ...
            try {
                ...
                if (authError.code === 'auth/email-already-in-use') {
                    console.log(`â„¹ï¸ User already exists, signing in: ${user.email}`);
            ...
            console.error(`âŒ Error handling user ${user.email}:`, error);
        }
    }
    return adminUserId;
}
...
async function seedPageViews() {
    console.log('ðŸŒ± Seeding page views...');
    ...
    for (const page of pages) {
        try {
            ...
            console.log(`âœ… Page view added for: ${page}`);
        } catch (error) {
            console.error(`âŒ Error adding page view for ${page}:`, error);
        }
    }
}

// Main seeding function
async function seedMaplotiKenya() {
    console.log('ðŸš€ Starting Firebase seeding for MaplotiKenya project...');
    await setupAuth();
    ...
    try {
        ...
        if (adminUserId) {
            ...
            console.error("âŒ Failed to get admin user ID, skipping property/agent seeding.");
        }
        ...
        console.log('\nðŸ”‘ Test user credentials are taken from the SEED_ADMIN_* / SEED_USER_* env vars');
