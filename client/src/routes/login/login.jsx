...
function PremiumInput({
  ...
  return (
    <div className="space-y-2">
      <label className={`block text-sm font-semibold tracking-wide transition-colors duration-300 ${isDark ? 'text-gray-200' : 'text-gray-700'
        }`}>
        {label}
      </label>

      <div className="relative group">
        <div className={`relative flex items-center border-2 rounded-xl px-6 py-4 transition-all duration-700 ease-out backdrop-blur-xl ${isDark
          ...
          {Icon && (
            <Icon className={`w-5 h-5 mr-4 transition-colors duration-700 ease-out ${isFocused
          ...
          <input
            ...
            className={`flex-1 bg-transparent outline-none text-base font-medium tracking-wide transition-colors duration-700 ease-out ${isDark ? 'text-gray-100 placeholder-gray-400' : 'text-gray-900 placeholder-gray-500'
          ...
          {type === 'password' && (
            <button
              ...
              disabled={disabled}
              className={`p-2 rounded-lg transition-all duration-700 ease-out ${isDark
        ...
        <div className={`absolute inset-0 pointer-events-none rounded-xl transition-all duration-700 ease-out ${isFocused
          ? isDark
...
function PremiumButton({
  ...
  return (
    <button
      ...
      className={`relative overflow-hidden rounded-full ${sizeClasses[size]} ${variantClasses[variant]} ${fullWidth ? 'w-full' : ''} flex items-center justify-center gap-3 ${disabled || loading ? 'opacity-50 cursor-not-allowed' : ''
        }`}
    >
      {loading ? (
...
function ModernLogin() {
  ...
  const { signIn, signInWithGoogleAuth, isVerifiedAgent, getUserRole, currentUser } = auth || {};
  ...
  // Redirect already-logged-in users to their role-specific page (skip if we just signed in on this page)
  useEffect(() => {
    if (!mounted || !currentUser || !getUserRole || justSignedInRef.current) return;
  ...
  }, [mounted, currentUser, getUserRole, navigate]);
  ...
  return (
    <div className={`min-h-screen transition-all duration-700 ease-out ${isDark
      ...
      {/* Navigation */}
      <div className="fixed left-6 z-50" style={{ top: 'calc(1.5rem + env(safe-area-inset-top))' }}>
        <button
          onClick={() => navigate('/desktop/register')}
          className={`px-8 py-4 font-semibold tracking-wide rounded-full transition-all duration-700 ease-out shadow-xl backdrop-blur-xl border ${isDark
      ...
      {/* Theme Toggle */}
      <div className="fixed right-6 z-50" style={{ top: 'calc(1.5rem + env(safe-area-inset-top))' }}>
        <button
          onClick={toggleTheme}
          className={`p-4 rounded-full transition-all duration-700 ease-out shadow-xl backdrop-blur-xl border ${isDark
            ? 'bg-gray-800/50 text-gray-200 border-gray-600/50 hover:bg-gray-700/50 hover:border-emerald-500/50 shadow-gray-900/20 hover:shadow-emerald-500/10'
            : 'bg-white/70 text-gray-700 border-gray-200/50 hover:bg-white/90 hover:border-emerald-500/50 shadow-gray-900/10 hover:shadow-emerald-500/10'
            }`}
        >
      ...
      <div className="relative min-h-screen flex items-center justify-center p-6">
        <div className="w-full max-w-xl">
          ...
          {/* User Type Selection */}
          <div className="mb-6">
            <div className={`rounded-full p-2 shadow-xl backdrop-blur-xl border transition-all duration-700 ease-out ${isDark
              ...
              <div className="flex">
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-3 px-6 py-4 rounded-full font-semibold tracking-wide transition-all duration-700 ease-out ${userType === 'user'
                    ? isDark
                ...
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-3 px-6 py-4 rounded-full font-semibold tracking-wide transition-all duration-700 ease-out ${userType === 'agent'
                    ? isDark
                      ? 'bg-emerald-600/80 text-white shadow-xl shadow-emerald-500/20'
                      : 'bg-emerald-600 text-white shadow-xl shadow-emerald-500/20'
                    : isDark
            ...
            {userType === 'agent' && (
              <div className={`mt-4 p-4 rounded-2xl border backdrop-blur-xl transition-all duration-700 ease-out ${isDark
                ? 'bg-emerald-900/20 border-emerald-700/50'
                : 'bg-emerald-50/50 border-emerald-200/50'
                }`}>
                <div className="flex items-start gap-3">
                  <Briefcase className={`w-5 h-5 mt-0.5 flex-shrink-0 ${isDark ? 'text-emerald-400' : 'text-emerald-600'
                    }`} />
                  <p className={`text-sm leading-relaxed ${isDark ? 'text-emerald-200' : 'text-emerald-700'
                    }`}>
                    {isVerifiedAgent
                      ? 'You are a verified agent. You will be redirected to the agent dashboard after sign-in.'
                      : 'Agent access requires verification after sign-in.'
                    }
                  </p>
                </div>
              </div>
          ...
          <div className="space-y-6">
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-3 cursor-pointer">
                  <input
                    type="checkbox"
                    className={`w-5 h-5 border-2 rounded focus:ring-2 transition-all duration-700 ease-out ${isDark
                  ...
                  />
                  <span className={`text-sm font-medium tracking-wide transition-colors duration-700 ease-out ${isDark ? 'text-gray-300' : 'text-gray-700'
                    }`}>
                    Remember me
                  </span>
                </label>
                <button
                  ...
                  className={`text-sm font-medium tracking-wide transition-colors duration-700 ease-out disabled:opacity-50 ${isDark ? 'text-gray-300 hover:text-emerald-400' : 'text-gray-700 hover:text-emerald-600'
                    }`}
                  disabled={isSubmitting}
                >
                  Forgot password?
            ...
            <div className="my-8 text-center">
              <span className={`text-sm font-medium tracking-wide transition-colors duration-700 ease-out ${isDark ? 'text-gray-400' : 'text-gray-500'
            ...
            {/* Sign up link */}
            <div className={`text-center pt-6 border-t transition-all duration-700 ease-out ${isDark ? 'border-gray-700/50' : 'border-gray-200/50'
              }`}>
              <p className={`mb-4 text-sm transition-colors duration-700 ease-out ${isDark ? 'text-gray-400' : 'text-gray-600'
                }`}>Don't have an account?</p>
              <button
                onClick={() => navigate('/desktop/register')}
                className={`text-base font-semibold tracking-wide transition-colors duration-700 ease-out disabled:opacity-50 ${isDark ? 'text-gray-300 hover:text-emerald-400' : 'text-gray-700 hover:text-emerald-600'
                  }`}
