...
const EnhancedMobilePropertyCard = ({
  ...
  const formatPricePerSqft = (price, area) => {
    ...
    return `Ksh ${pricePerSqft}/sqft`;
  };

  // Get property status
  const getPropertyStatus = () => {
    const s = property.status?.toLowerCase() || '';
    if (s === 'sold') return { label: 'Sold', color: 'bg-red-500', icon: CheckCircle };
  ...
  return (
    <motion.div
      ref={cardRef}
      className={`group bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden transition-all duration-300 cursor-pointer hover:shadow-xl lg:hidden ${isDark ? 'border border-gray-700/50' : 'border border-gray-200/50'
          ...
          <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 flex space-x-1">
            {property.images.map((_, index) => (
              <motion.button
                ...
                className={`w-2 h-2 rounded-full transition-all ${index === imageIndex
                  ? 'bg-white shadow-lg'
        ...
        {showActions && (
          <div className="absolute top-3 right-3 flex space-x-2">
            <motion.button
              ...
              className={`w-8 h-8 rounded-full flex items-center justify-center backdrop-blur-sm ${isLiked
          ...
          <div className="flex items-center justify-between">
            ...
            {property.priceChange && (
              <motion.div
                initial={{ opacity: 0, scale: 0.8 }}
                animate={{ opacity: 1, scale: 1 }}
                className={`flex items-center space-x-1 px-2 py-1 rounded-full text-xs font-semibold ${property.priceChange > 0
                  ? 'bg-[#4066ff]/20 text-[#4066ff] dark:bg-[#4066ff]/20 dark:text-[#40f2ff]'
            ...
            <span className="text-sm">
              {(() => {
                ...
                if (typeof property.address === 'object') {
                  return property.address.address || property.address.city || 'Location available';
                }
          ...
          >
            ...
            {property.description.length > 100 && (
              <button
                ...
                className="text-sm text-[#4066ff] hover:text-[#40f2ff] mt-1"
              >
        ...
        {property.agent && (
          ...
          >
            ...
            {property.agent.verified && (
              <Shield className="w-4 h-4 text-[#4066ff]" />
            )}
          </motion.div>
        )}
