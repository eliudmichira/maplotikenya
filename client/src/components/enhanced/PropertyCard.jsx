...
const PropertyCard = ({
  ...
  if (viewMode === 'list') {
    return (
      <motion.div
        ref={cardRef}
        className={`group relative bg-white dark:bg-gray-800 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden border border-gray-100 dark:border-gray-700 ${isHighlighted ? 'ring-2 ring-[#4066ff] ring-opacity-50' : ''
          }`}
      ...
      >
        <div className="flex flex-col md:flex-row h-auto md:h-56">
          {/* Image Section */}
          <div className="relative w-full h-64 md:h-full md:w-80 flex-shrink-0">
            <motion.div
            ...
            >
              ...
              >
                ...
                >
                  <Heart
                    className={`w-4 h-4 transition-colors duration-300 ${isFavorite ? 'text-red-500 fill-red-500' : 'text-gray-600 dark:text-gray-400'
                      }`}
                  />
                </motion.button>
                <motion.button
          ...
          {/* Content Section */}
          <motion.div
            className="flex-1 p-4 md:p-6 flex flex-col justify-between gap-4 md:gap-0"
      ...
      >
        ...
        <div className="mb-3">
          <h4 className="text-lg font-semibold text-gray-900 dark:text-white mb-1 line-clamp-1">
            {property.title || (typeof property.address === 'string' ? property.address : (property.address?.address || property.address?.city || 'Location'))}
          </h4>
          <div className="flex items-center gap-1 text-gray-600 dark:text-gray-400">
            <MapPin className="w-4 h-4 flex-shrink-0" />
            <span className="text-sm line-clamp-1">
