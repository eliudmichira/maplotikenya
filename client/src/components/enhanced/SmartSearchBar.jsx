...
const SmartSearchBar = ({
  ...
  const handleSearch = (query) => {
    ...
    if (searchText.trim()) {
      // Add to recent searches (unified object format)
      const newSearch = {
        id: Date.now(),
      ...
      const newRecent = [
        newSearch,
        ...recentSearches.filter(s => (typeof s === 'string' ? s : s.text) !== searchText.trim())
      ].slice(0, 5);
    ...
    <div className="relative w-full">
      ...
      <AnimatePresence>
        {showSuggestions && (isFocused || searchQuery) && (
          <motion.div
            className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-gray-800 rounded-3xl shadow-2xl border border-gray-200 dark:border-gray-700 z-[100] max-h-96 overflow-y-auto"
              ...
              <div className="p-4 border-b border-gray-200 dark:border-gray-700">
                ...
                <div className="space-y-1">
                  {recentSearches.map((search, index) => {
                    const searchText = typeof search === 'string' ? search : search.text;
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-gray-700 dark:text-gray-300">{searchText}</span>
                      </motion.button>
                    );
                  })}
                </div>
