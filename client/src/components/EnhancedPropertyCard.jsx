...
const EnhancedPropertyCard = ({ property, onFavoriteToggle, isFavorite, onVacancyUpdate }) => {
  ...
  return (
    <div className={`group bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden transition-all duration-300 cursor-pointer hover:shadow-xl ${isDark ? 'border border-gray-700' : 'border border-gray-200'
      }`}>
      {/* Image Section */}
      <div className="relative overflow-hidden h-48">
        ...
        <div className="absolute top-3 right-3 flex flex-col space-y-2">
          <button
            ...
            className={`w-10 h-10 rounded-full flex items-center justify-center transition-all shadow-lg ${isFavorite
      ...
      <div className="p-4">
            ...
            <span className="line-clamp-1">
              {(() => {
                ...
                if (typeof addr === 'object' && addr !== null) {
                  return addr.address || addr.city || 'Location not specified';
                }
                return property.location?.address || 'Location not specified';
              })()}
            </span>
          </div>
        </div>
        ...
        <div className="flex space-x-2">
          <Link
            ...
            className="flex-1 flex items-center justify-center space-x-2 py-3 bg-gradient-to-r from-[#fbbf24] to-[#f59e0b] text-[#000000] rounded-xl font-semibold transition-all hover:shadow-lg"
          >
