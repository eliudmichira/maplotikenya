...
// Function to resolve Google Maps short URLs
exports.resolveShortUrl = functions.https.onCall(async (data, context) => {
  ...
  // Only allow Google Maps short URLs for security
  if (!url.includes('maps.app.goo.gl') && !url.includes('goo.gl/maps')) {
    throw new functions.https.HttpsError('invalid-argument', 'Only Google Maps short URLs are allowed');
  }

  try {
    // Use axios to follow redirects and get the final URL
    ...
    throw new functions.https.HttpsError('internal', 'Failed to resolve short URL');
  }
});
...
const sendTrialWelcomeEmail = async (emailData) => {
  ...
  const emailTemplate = `
    ...
    <head>
      ...
      <title>Welcome to RentBuyKenya Trial</title>
      <style>
        body { 
    ...
    <body>
      <div class="container">
        <div class="header">
          <h1 style="color: white; margin: 0; font-size: 28px;">Welcome to Rental Management! 🎉</h1>
        ...
        <div class="content">
          ...
          <p style="color: #555; line-height: 1.6;">Congratulations! Your Rental Management trial account has been created successfully. You now have access to all premium features for the next 30 days.</p>
...
const sendSalesNotification = async (emailData) => {
  ...
  const salesEmailTemplate = `
    ...
    <head>
      <meta charset="utf-8">
      <title>New RentBuyKenya Trial Signup</title>
      <style>
